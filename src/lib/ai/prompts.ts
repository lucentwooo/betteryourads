...
export function buildCategoryPrompt(
  ...
  return `Analyze this company's website and identify:
1. What CATEGORY of business/product this is (3-7 words — the phrase someone would Google to find this type of thing)
2. Whether this is a LOCAL service business (tied to a specific city/region — e.g. landscaping, plumbing, dentist, gym, restaurant, HVAC, contractor) vs a NATIONAL product (SaaS, app, ecommerce brand, DTC product)
3. If local, what city/metro area they serve
...
Examples:
- Tally -> {"category": "online form builder", "isLocal": false, "city": null}
- Fathom -> {"category": "AI meeting notes app", "isLocal": false, "city": null}
- "Green Thumb Landscaping, Seattle" -> {"category": "residential landscaping service", "isLocal": true, "city": "Seattle"}
- "Dr. Smith Dental, Austin" -> {"category": "family dentist", "isLocal": true, "city": "Austin"}

Return ONLY a JSON object, no markdown, no extra text:
{"category": "...", "isLocal": true|false, "city": "City Name" | null}`;
}
...
export function buildCompetitorSuggestionPrompt(
  ...
  if (isLocal && city) {
    return `The company we're researching is ${companyName} — a LOCAL ${category} business serving ${city}.

List the top 3 REAL, DIRECT local competitors: other independent ${category} businesses actually operating in ${city} (or the ${city} metro area) that a customer would realistically choose between.

Think like you Googled "best ${category} in ${city}" and then filtered the results to ONLY true direct competitors.

HARD EXCLUSIONS — do NOT include any of these:
- Big-box / national retailers (Home Depot, Lowe's, Walmart, Costco, IKEA, etc.)
- Directories, marketplaces, or lead-gen aggregators (Angi / Angie's List, Yelp, Thumbtack, HomeAdvisor, TaskRabbit, Houzz, Nextdoor, Google, Bing, Porch, Bark)
- Franchises or national chains unless they have a specific local branch that genuinely competes head-to-head
- Media / review sites / blogs / "top 10" lists
- ${companyName} itself
- Generic or made-up names — only real businesses you're confident exist in ${city}

Each competitor must be:
- An independent ${category} business
- Actually located in or serving ${city}
- Something a real local customer would compare ${companyName} against
- Likely to have a Facebook Page (so we can search Meta Ad Library)

If you're not confident a specific local business exists, return fewer than 3 rather than inventing names.

Return ONLY a JSON array, no markdown, no extra text:
[{"name": "Exact Business Name", "searchTerm": "Exact Business Name ${city}"}]`;
  }

  return `List the top 3 most popular apps/tools/brands in the "${category}" category that are most likely running Meta (Facebook/Instagram) ads.
...
- Do NOT include directories, marketplaces, or aggregators (Yelp, Angi, Amazon, Home Depot, etc.) — only direct product/brand competitors

Think: if someone Googled "best ${category}" or "top ${category}", which 3 brands would dominate the results (excluding ${companyName} and excluding aggregators/retailers)?
